...
async fn scan() -> Vec<jojo_common::network::Ssid> {
    ...
    match tauri_sys::tauri::invoke::<_, jojo_common::otp::ScanResponse>("scan", &()).await {
        ...
        Err(err) => {
            ...
            error!("[scan]: something went wrong {:?}", err);
...
// TODO: implement a retry logic
async fn save_credentials(ssid: String, password: String) -> anyhow::Result<()> {
    ...
    {
        Ok(_) => {
            log!("[save_credentials]: saved");
            Ok(())
        }
        Err(err) => {
            // TODO: think what to do in error
            error!("[save_credentials]: something went wrong {:?}", err);
            Err(anyhow!("{:?}", err))
        }
    }
}
...
#[component]
pub fn OtpStep<F>(on_success: F) -> impl IntoView
where
    F: Fn() + 'static + Clone,
{
    ...
    let save_action = create_action(move |credential: &(String, String)| {
        let (ssid, password) = credential.clone();
        log!("Action: connect to client, {:?}:{:?}", ssid, password);
        let on_success = on_success.clone();

        async move {
            match save_credentials(ssid, password).await {
                Ok(_) => on_success(),
                Err(_) => todo!(),
            }
        }
    ...
    let ssid_resources = create_resource(|| (), |_| async move { scan().await });
    ...
    let dispatch_action = move || {
        ...
        save_action.dispatch((ssid.get(), password.get()));
    };
